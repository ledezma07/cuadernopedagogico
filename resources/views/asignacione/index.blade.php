<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('ASIGNACION DE MATERIAS A DOCENTES') }}
        </h2>
    </x-slot>
    <!DOCTYPE html>
    <html lang="en">
    ...
    <div class="container mx-auto px-4 py-4 bg-blue-000">
        <section class="py-4">
            <div class="w-1/2 mx-4 ">
                <form action="{{ route('asignacione.create') }}">
                  ...
                  <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                    <div class="overflow-x-auto">
                      <table
                        ...
                        <tbody>
                          @foreach ($asignacione as $asignacione) 
                          <tr class="border-b border-neutral-800 dark:border-white/10">
                            <td class="whitespace-nowrap px-6 py-4 font-medium">{{$asignacione->id}}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{$asignacione->fecha}}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{$asignacione->periodo_id}}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{$asignacione->docentre_id}}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{$asignacione->curso_id}}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{$asignacione->usuario_id}}</td>
